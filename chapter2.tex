...
Because sine and cosine are continuous periodic functions, mapping absolute day values onto these waves ensures that the mathematical distance
between any two encoded time steps is proportional to the actual temporal gap between them.
By calculating the encoding angles based on this absolute temporal position, the model inherently understands whether consecutive satellite acquisitions are 5 days apart or 30 days apart.
This explicit temporal injection ensures the network accurately aligns the spectral data with the correct stage of the crop's phenological cycle.
...
As detailed in Algorithm \ref{alg:agrodays}, the implementation relies on several key intermediate variables to achieve this projection.
The $div\_term$ represents the frequency scaling tensor, calculated logarithmically to ensure a geometric progression
of wavelengths across the network's core embedding dimension ($d\_model$, defined as 128 in this architecture).
The input matrix $pos$, containing the absolute agricultural days, is broadcasted into $pos\_expanded$ and multiplied by the frequency scales to generate the $angles$ tensor.
Finally, applying the sine and cosine operations to even and odd indices respectively yields the final positional encoding tensor, $PE$, which is added directly to the input sequence.
...
Driven by Multi-Head Self-Attention \cite{attention_is_all_you_need}, an Intra-Year Transformer Encoder evaluates the entire yearly sequence simultaneously,
...
In the second stage, the architecture transitions from intra-year phenology to inter-year crop dependencies
by passing the extracted features into an Inter-Year Long Short-Term Memory (LSTM) network \cite{hochreiter_long_1997}.
...
Algorithm \ref{alg:hybrid_forward} outlines the exact tensor transformations required to bridge these two temporal scales. 
To optimize hardware efficiency during the intra-year stage, the raw sequence $X$ is flattened into $X_{flat}$, temporarily merging the batch ($B$) and year ($Y$) dimensions. 
This critical step allows the Transformer to process all years independently and in parallel across the GPUs. 
The Transformer's output sequence, $T_{out}$, is then temporally mean-pooled into $Y_{emb}$,
collapsing the complex intra-year phenology into a single, dense feature vector for each specific year. 
Finally, the chronological sequence is restored ($Y_{seq}$) and passed to the LSTM, which outputs the deeply fused multi-year
features $L_{out}$ alongside its final hidden and cell states ($h_n, c_n$).
